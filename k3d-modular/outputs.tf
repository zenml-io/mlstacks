...
output "orchestrator_id" {
  value = var.enable_kubeflow || var.enable_kubernetes || var.enable_tekton ? uuid() : ""
}
output "orchestrator_flavor" {
  value = var.enable_kubeflow ? "kubeflow" : var.enable_kubernetes ? "kubernetes" : var.enable_tekton ? "tekton" : ""
}
output "orchestrator_name" {
  value = var.enable_kubeflow ? "k3d-kubeflow-${random_string.cluster_id.result}" : var.enable_kubernetes ? "k3d-kubernetes-${random_string.cluster_id.result}" : var.enable_tekton ? "k3d-tekton-${random_string.cluster_id.result}" : ""
}
...
output "experiment_tracker_name" {
  value = var.enable_mlflow ? "k3d-mlflow-${random_string.cluster_id.result}" : ""
}
...
output "model_deployer_id" {
  value = var.enable_seldon || var.enable_kserve ? uuid() : ""
}
output "model_deployer_flavor" {
  value = var.enable_seldon ? "seldon" : var.enable_kserve ? "kserve" : ""
}
output "model_deployer_name" {
  value = var.enable_seldon ? "k3d-seldon-${random_string.cluster_id.result}" : var.enable_kserve ? "k3d-kserve-${random_string.cluster_id.result}" : ""
}
output "model_deployer_configuration" {
  value = var.enable_seldon ? jsonencode({
    kubernetes_context = "k3d-${k3d_cluster.zenml-cluster[0].name}"
