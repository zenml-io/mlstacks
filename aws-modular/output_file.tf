...
%{if var.enable_tekton}
        ...
        configuration: {"kubernetes_context": "terraform"}
%{else}
...
%{if var.enable_kserve}
      model_deployer:
        ...
        configuration: {"kubernetes_context": "terraform", "kubernetes_namespace": "${local.kserve.workloads_namespace}", "base_url": "${module.kserve[0].kserve-base-URL}", "secret": "aws_kserve_secret"}
%{endif}
%{if var.enable_seldon}
      model_deployer:
        ...
        configuration: {"kubernetes_context": "terraform", "kubernetes_namespace": "${local.seldon.workloads_namespace}", "base_url": "http://${module.istio[0].ingress-hostname}:${module.istio[0].ingress-port}"}}
%{endif}
    ADD
