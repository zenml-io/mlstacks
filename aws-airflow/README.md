# 🥗 AWS MWAA Airflow MLOps Stack Recipe 
...
- A managed Airflow environment provisioned through AWS MWAA that can be used as [orchestrator](https://docs.zenml.io/mlops-stacks/orchestrators) for your pipelines.
...
1. Pull this recipe to your local system.
    ...
    ```shell
    zenml stack recipe pull aws-airflow
    ```
...
5. 🚀 Deploy the recipe with this simple command.
    ...
    ```
    zenml stack recipe deploy aws-airflow
    ```
...
| Output | Description |
--- | ---
airflow-uri | The URL of the Airflow UI. |
s3-bucket-path | The path of the S3 bucket. Useful while registering the artifact store|
container-registry-uri | The URI of the container registry |
...
1. 🗑️ Run the destroy command which removes all resources and their dependencies from the cloud.
    ...
    ```shell
    zenml stack recipe destroy aws-airflow
    ```
